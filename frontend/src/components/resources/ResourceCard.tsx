...
const ResourceCardComponent: React.FC<ResourceCardProps> = ({
  ...
  const uploadedBy = resource.uploadedBy || { id: 0, displayName: 'Unknown User' };
  const course = resource.course || { id: 0, name: 'Unknown Course' };

  const isOwner = user?.id === uploadedBy.id;
  ...
  return (
    <Card hover className="group">
      <div className="p-6">
        ...
        {/* Resource title */}
        <h3
          className={`text-lg font-semibold text-text-primary mb-3 line-clamp-2 leading-snug ${resource.url ? 'cursor-pointer hover:text-primary transition-colors group-hover:text-primary' : ''
            }`}
        ...
        {showCourse && (
          <p className="text-sm text-text-secondary mb-3 font-medium">
            <span className="text-gray-500">Course:</span> {course.name}
          </p>
        )}
        ...
        <div className="flex items-center justify-between text-sm text-text-secondary mt-auto">
          <div className="flex flex-col gap-1">
            <span className="font-medium">{uploadedBy.displayName}</span>
            {resource.createdAt && (
